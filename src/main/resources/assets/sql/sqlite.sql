...
CREATE TABLE IF NOT EXISTS bank_currencies (
    bank_currency_id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE IF NOT EXISTS exchange_rules (
    exchange_rule_id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE IF NOT EXISTS central_banks (
    central_bank_id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE,
    owner_id INTEGER REFERENCES users(user_id),
...
CREATE TABLE IF NOT EXISTS central_banks_countries (
    central_bank_country_id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE IF NOT EXISTS accounts_currencies (
    account_currency_id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE VIEW IF NOT EXISTS v_users_accounts_banks AS
    SELECT
...
CREATE VIEW IF NOT EXISTS v_accounts_currencies AS
    SELECT
...
CREATE VIEW IF NOT EXISTS v_accounts_balances AS
    SELECT
...
CREATE VIEW IF NOT EXISTS v_bank_currencies AS
    SELECT
...
CREATE TRIGGER IF NOT EXISTS accounts_currencies_checker
    BEFORE INSERT
    ON accounts_currencies FOR EACH ROW
    WHEN NEW.currency_id NOT IN (
...
BEGIN
    SELECT RAISE(FAIL, 'Cannot insert this account currency: the bank does not support the currency');
END;

-- Create trigger "bank_balance_updater", that updates tha bank's balances on user's accounts balance update
CREATE TRIGGER IF NOT EXISTS bank_balance_updater
    AFTER UPDATE
    ON accounts_currencies FOR EACH ROW
BEGIN
